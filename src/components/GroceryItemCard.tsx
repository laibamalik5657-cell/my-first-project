...
function GroceryItemCard({ item }: { item: IGrocery }) {
    ...
    return (
        <motion.div
            ...
            className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group bg-gray-100 flex flex-col"
        >
            ...
            <div className="p-4 flex flex-col flex-1">
                ...
                {!cartItem ? 
                <motion.button 
    className='mt-4 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white rounded-full py-2 text-sm font-medium transition-all w-full'
    whileTap={{ scale: 0.96 }}
...
>
  ...
  >
    {/* Minus Button */}
    <button 
      className="w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all" onClick={()=>dispatch(decreaseQuantity((item._id.toString())))}
    >
    ...
    {/* Plus Button */}
    <button 
      className="w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all" onClick={()=>dispatch(increaseQuantity((item._id.toString())))}
    >
