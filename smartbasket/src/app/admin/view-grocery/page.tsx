...
>
    ...
    <h1 className='text-2xl font-extrabold text-gray-800 md:text-3xl text-green-700
     flex items-center justify-center gap-2'>
        <Package size={18} className='text-green-600'/> Manage Groceries</h1>
</motion.div>
<motion.form 
...
<div className='space-y-4'>
  {groceries?.map((g, i) => (
    ...
    >
      <div className='relative w-full sm:w-44 aspect-square rounded-xl 
...
<div className='mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
  <p className='text-green-700 font-bold text-lg'>
    Rs. {g.price} / <span className='text-gray-500 text-sm font-medium ml-1'>{g.unit}</span>
  </p>
<button className='bg-green-600 text-white px-4 py-2 rounded-lg text-font-semibold flex items-center justify-center gap-2
 hover:bg-green-700 transition-all' onClick={()=>setEditing(g)}>
    <Pencil size={15} />  Edit
</button>
</div>
</div>
...
<div className='relative aspect-square w-full rounded-lg overflow-hidden mb-4 border border-gray-200 group'>
  {imagePreview && 
    <Image 
      src={imagePreview} 
      alt={editing?.name || 'Grocery'} 
      fill 
      className='object-cover' 
    />
  }
  <label htmlFor='imageUpload' className='absolute inset-0 bg-black/40 opacity-0
group-hover:opacity-100 flex items-center justify-center cursor-pointer
transition-opacity'>
...
<div className="flex justify-end gap-3 mt-6">
    ...
    <button className="px-4 py-2 rounded-lg bg-red-600 text-white flex 
    items-center gap-2 hover:bg-red-700 transition" onClick={handleDelete}
