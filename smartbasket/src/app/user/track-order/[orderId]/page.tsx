...
if(message.roomId==orderId){
  ...
  return()=>{socket.off("send-messages")}
},[])
    ...
    <div className='sticky top-0 bg-white/80 backdrop-blur-xl p-4 border-b shadow flex gap-3 items-center z-999'>
      ...
      <div >
        ...
        <p className='text-sm text-gray-600'>
          Order #{order?._id?.toString().slice(-6)} <span className='text-green-700 font-semibold'>{order?.status}</span>
        </p>
      </div>
    </div>
...
<div className='flex-1 overflow-y-auto p-2 space-y-3' ref={chatBoxRef}>
  <AnimatePresence>
    {messages?.map((msg, index) => (
      ...
      >
        ...
        <div className={`px-4 py-2 max-w-[75%] rounded-2xl shadow ${msg.senderId.toString()=== userData?._id?
          ...
          <p >{msg.text}</p>
          <p className='text-right text-[10px] opacity-70 mt-1'>{msg.time}</p>
        </div>
      </motion.div>
    ))}
  </AnimatePresence>
    ...
    <input type="text" placeholder="Type a Message..." className='flex-1 bg-gray-100 px-4 py-2 rounded-xl outline-none focus:ring-2
