  ...
  return (
    <div className="w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative">
      ...
      <motion.h2
        ...
        className='text-2xl sm:text-3xl md:text-4xl font-bold text-green-700 text-center'
      >
        Your Shopping Cart
      </motion.h2>
              ...
              {cartData.map((item: any, index: number) => (
                ...
                >
                  ...
                  <div className='flex-1'>
                    ...
                    <p className="text-xs sm:text-xs text-gray-500">{item.unit}</p>
                    ...
                    <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0'>
                      <div className='bg-gray-50 px-3 py-2 rounded-full'>
                        <button className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-1 border border-gray-200' onClick={()=>dispatch(decreaseQuantity(item._id))}>
                          <Minus size={16} className='text-gray-600' />
                        </button>
                        <span className='font-semibold text-gray-800 w-6 text-center inline-block'>
                          {item.quantity}
                        </span>
                        <button className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-1 border border-gray-200'  onClick={()=>dispatch(increaseQuantity(item._id))}>
    ...
    <button className="bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200" onClick={()=>dispatch(decreaseQuantity(item._id))}>
        <Minus className='size={14} text-green-700'/>
    </button>
    <span className='font-semibold text-gray-800 w-6 text-center'>{item.quantity}</span>
    <button className="bg-white p-1.5 rounded-full hover:bg-green-100 transition border border-gray-200" onClick={()=>dispatch(increaseQuantity(item._id))}>
...
<motion.button whileTap={{scale:0.9}} className='w-full mt-6 bg-green-600 text-white py-3 rounded-full
 hover:bg-green-700 transition-all font-semibold text-sm sm:text-base' onClick={()=>router.push("/user/checkout")}>Proceed to Checkout</motion.button>
</motion.div>
</div>
      )}
      </div>
